...
// SC: optional setting to unlock wallet for block minting only;
...
// SC: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // SC: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // SC: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    //  post-backup change.
#ifdef WIN32
                    if (!GetBoolArg("-avatar", true)) // SC: not avatar mode; sc: avatar mode enabled by default to avoid change being sent to hidden address
#else
...
// SC: create coin stake transaction
...
// SC: check 'spent' consistency between wallet and txindex
// SC: fix wallet spent state according to txindex
...
// SC: disable transaction (only for coinstake)
