...
{
    // Wait for IRC to get it first - disabled with sc
...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "sc " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
// testnet dns seed begins with 't', all else are sc dns seeds.
static const char *strDNSSeed[][3] = {
    // NOTE: Only testnet nodes are included in this build. If you are using this
    // for a real sc offering, you will want to remove the "t" that
    // appears before the node name and IP
       {"182.254.141.46","115.159.41.207","123.206.201.165"}
};
...
void ThreadDNSAddressSeed2(void* parg)
{
    ...
    if (true /*!fTestNet*/)  // SC enables dns seeding with testnet too
    {
...
bool BindListenPort(string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, sizeof(sockaddr)) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to port %d on this computer.  sc is probably already running."), ntohs(sockaddr.sin_port));
        else
...
void StartNode(void* parg)
{
    ...
    if(GetBoolArg("-genpos", false)){
        // SC: mint proof-of-stake blocks in the background
